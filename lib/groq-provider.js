...
function getGroqClient() {
  if (!groqClient && (process.env.Groq_API || process.env.GROQ_API_KEY)) {
    groqClient = new Groq({
...
async function generateResponse({ message, intern, destination }) {
  ...
  try {
    ...
    if (!apiKey) {
      console.warn('Groq API key not configured (checked Groq_API and GROQ_API_KEY), skipping Groq provider');
      return null;
    }
