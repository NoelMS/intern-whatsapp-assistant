...
function keywordFallbackSearch(query, faqs, topK = 3) {
  ...
  // First, check for specific phrase patterns in the query
  const isEatingQuery = queryLower.includes('eat') || queryLower.includes('food') || queryLower.includes('restaurant') || queryLower.includes('dining');
  const isAirportQuery = queryLower.includes('airport') && !queryLower.includes('eat');
  const isAirportDiningQuery = queryLower.includes('airport') && queryLower.includes('eat');
  
  console.log(`Query type - eating: ${isEatingQuery}, airport: ${isAirportQuery}, airport+eating: ${isAirportDiningQuery}`);
  
  // If query mentions eating/food, prioritize dining FAQs even if airport is mentioned
  if (isEatingQuery) {
    console.log('Prioritizing dining FAQs');
    
    const diningFAQs = faqs.map(faq => {
      const questionLower = faq.question.toLowerCase();
      const answerLower = faq.answer.toLowerCase();
      let score = 0;
      
      // Strong bonus for eating/dining related questions
      if (questionLower.includes('places to eat') || questionLower.includes('restaurant') || questionLower.includes('dining')) {
        score += 100;
      }
      ...
      // Bonus for having eat/food/restaurant keywords
      if (questionLower.includes('eat')) score += 30;
      if (questionLower.includes('restaurant')) score += 30;
      if (questionLower.includes('food')) score += 30;
      if (questionLower.includes('cafe')) score += 20;
      
      // Penalty if FAQ is about other topics
      if (questionLower.includes('airport') && !questionLower.includes('eat')) score -= 50;
      if (questionLower.includes('curfew') || questionLower.includes('hostel')) score -= 30;
  ...
  // If query is about airport transportation, prioritize airport FAQs
  if (isAirportQuery) {
    console.log('Prioritizing airport FAQs');
    
    const airportFAQs = faqs.map(faq => {
      const questionLower = faq.question.toLowerCase();
      let score = 0;
      
      if (questionLower.includes('airport')) score += 100;
      if (questionLower.includes('transport')) score += 50;
      if (questionLower.includes('get from')) score += 30;
  ...
  const topics = {
    dining: {
      priority: 1,
      keywords: ['eat', 'food', 'restaurant', 'cafe', 'dining', 'meal']
    },
    airport: {
      priority: 2,
    ...
    hospital: {
      priority: 4,
      keywords: ['hospital', 'medical', 'doctor', 'health']
    },
    transport: {
      priority: 5,
      keywords: ['travel', 'bus', 'train', 'taxi', 'auto']
    },
    hostel: {
      priority: 6,
      keywords: ['hostel', 'room', 'curfew', 'warden']
    },
    emergency: {
      priority: 7,
  ...
  console.log(`Primary topic: ${primaryTopic} (${maxMatches} matches)`);
  ...
  const scoredFAQs = faqs.map(faq => {
    ...
    if (primaryTopic) {
      const topicKeywords = topics[primaryTopic].keywords;
      const questionMatches = topicKeywords.filter(kw => questionLower.includes(kw)).length;
      const answerMatches = topicKeywords.filter(kw => answerLower.includes(kw)).length;
      
      score += questionMatches * 20;
      score += answerMatches * 10;
    }
