...
function keywordFallbackSearch(query, faqs, topK = 3) {
  ...
  const topics = {
    ...
    airport: ['airport', 'flight', 'arrival', 'departure', 'transport from airport'],
    wifi: ['wifi', 'internet', 'password', 'network', 'connection', 'internet'],
    hospital: ['hospital', 'medical', 'emergency', 'doctor', 'health', 'medicine'],
    transport: ['travel', 'bus', 'train', 'taxi', 'auto', 'transport', 'ooty', 'munnar', 'coonoor'],
    hostel: ['hostel', 'room', 'accommodation', 'curfew', 'warden', 'check-in', 'facilities', 'bed'],
    emergency: ['emergency', 'police', 'ambulance', 'help', 'danger', 'crisis'],
    banks: ['bank', 'atm', 'money', 'financial', 'account']
  };
  
  // Score each topic based on how many keywords appear in the query
  const topicScores = {};
  ...
  const primaryScore = sortedTopics[0]?.[1] || 0;
  
  console.log(`Detected primary topic: ${primaryTopic} (score: ${primaryScore})`);
  ...
  const scoredFAQs = faqs.map(faq => {
    ...
    if (primaryTopic) {
      const topicKeywords = topics[primaryTopic];
      ...
      // Heavy bonus for matching primary topic in question
      score += questionTopicMatches * 10;
      // Medium bonus for matching primary topic in answer
      score += answerTopicMatches * 5;
      
      // PENALTY for matching other topics strongly (to avoid cross-matching)
      for (const [otherTopic, otherKeywords] of Object.entries(topics)) {
        if (otherTopic !== primaryTopic) {
          const otherMatches = otherKeywords.filter(kw => questionLower.includes(kw)).length;
          if (otherMatches > 0) {
            // Only penalize if it's a different major topic with strong matches
            score -= otherMatches * 3;
          }
        }
      }
    }
    
    // General word-level matching (medium weight)
    queryWords.forEach(word => {
      // Avoid giving points for very common words
      if (['near', 'next', 'close', 'the', 'and', 'or', 'in', 'on', 'at', 'to'].includes(word)) {
        return;
      }
