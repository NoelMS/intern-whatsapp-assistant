...
async function searchFAQsSemantically(query, faqs, topK = 3) {
  try {
    ...
    console.log(`Attempting semantic search for ${faqs.length} FAQs...`);
    ...
    try {
      ...
      // If semantic search worked, return results
      if (results.length > 0) {
        return results;
      }
    } catch (embedError) {
      console.log(`Semantic search failed (${embedError.message}), falling back to keyword search...`);
    }
    
    // Fall back to keyword-based search
    const keywordResults = keywordFallbackSearch(query, faqs, topK);
    console.log(`Keyword fallback found ${keywordResults.length} matches`);
    return keywordResults;

  } catch (error) {
    console.error('FAQ search error:', error.message);
    // Emergency fallback: try keyword search
...
function keywordFallbackSearch(query, faqs, topK = 3) {
  ...
  const scoredFAQs = faqs.map(faq => {
    ...
    // 1. PRIMARY TOPIC BONUS (highest weight)
    if (primaryTopic) {
    ...
    // 2. PENALTY for matching OTHER topics strongly (prevents cross-topic match)
    ...
    // 3. GENERAL WORD MATCHES (lower weight)
    // Only count meaningful words
    const meaningfulWords = ['airport', 'campus', 'city', 'hostel', 'room', 'center', 'office'];
