...
async function handleIncomingMessage({ from, to, body, messageSid }) {
  ...
  try {
    ...
    if (faqMatch) {
      ...
      // If AI fails, use a helpful generic response instead
      console.log(`ℹ️ No FAQ match, using generic response`);
      reply = `Hi ${intern.name}! 👋 

I'm the Intern Support Assistant. I don't have a specific answer to your question in my FAQ database.

Please reach out to your coordinator **${destination.coordinator.name}** at **${destination.coordinator.phone}** for personalized assistance.
...
For emergencies: **${destination.emergency_info.local_police}** (Police) or **${destination.emergency_info.ambulance}** (Ambulance)`;
