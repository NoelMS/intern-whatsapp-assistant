...
const { sendWhatsAppMessage } = require('./meta-whatsapp');
const { getInternByPhone, getDestination, searchFAQs } = require('./data-loader');
const { checkRateLimit } = require('./rate-limiter');
const { orchestrateAIResponse } = require('./ai-orchestrator');
...
async function handleIncomingMessage({ from, to, body, messageSid }) {
  ...
  try {
    ...
    // Greeting detection - catch common greetings
    const greetings = ['hi', 'hello', 'hey', 'howdy', 'hii', 'helo', 'hai', 'sup', 'good morning', 'good evening', 'good afternoon'];
    ...
    // First, try keyword-based FAQ search (no API calls needed)
    let reply;
    let responseType;
    
    console.log(`Searching for FAQ match...`);
    const faqMatch = searchFAQs(intern.destination_id, messageText);
    
    if (faqMatch && faqMatch.score >= 30) {
      // Strong FAQ match found
      console.log(`FAQ match found: ${faqMatch.id} (score: ${faqMatch.score})`);
      reply = faqMatch.answer;
      responseType = 'faq';
    } else {
      // No strong FAQ match, use AI provider orchestration
      console.log(`No strong FAQ match (score: ${faqMatch?.score || 0}), trying AI providers...`);
      reply = await orchestrateAIResponse({
        message: messageText,
        intern,
        destination
      });
