// GitHub Models AI integration with semantic search
const GITHUB_TOKEN = process.env.GITHUB_TOKEN;
const GITHUB_MODELS_API_URL = 'https://models.inference.ai.azure.com/chat/completions';
const { getRelevantContext } = require('./vector-search');
...
async function generateAIResponse({ message, intern, destination }) {
  try {
    ...
    const relevantContext = await getRelevantContext(message, destination.id);

    // Build context prompt
    const context = buildContextPrompt(intern, destination, relevantContext);
    ...
    if (!response.ok) {
      ...
      console.error('GitHub Models API response not ok:', response.status, errorMsg);
...
function buildContextPrompt(intern, destination, relevantContext = '') {
  const contextSection = relevantContext ? `\n\nRELEVANT INFORMATION (from FAQs):\n${relevantContext}\n` : '';
...
- Check-in Date: ${destination.accommodation.check_in}${contextSection}
...
- Do not use any markdown, bold, or special formatting
